...
def hg_propagate(new_g, tgt_type, num_hops, max_hops, extra_metapath, echo=True):
...
def load_dataset(args):
    if args.dataset == 'ogbn-products':
        ...
        return load_mag240m(args)
    else:
        assert 0, 'Only allowed [ogbn-products, ogbn-proteins, ogbn-arxiv, ogbn-papers100M, ogbn-mag, mag240m]'
...
def load_mag(args, symmetric=True):
    ...
    if symmetric:
        adjs[2] = adjs[2].to_symmetric() # paper cite paper to_symmetric
...
def load_mag240m(args, symmetric=True):
    ...
    g = dgl.heterograph({
        ('A', 'A-P', 'P'): (ei_writes[0], ei_writes[1]),
        ('A', 'A-I', 'I'): (ei_affiliated[0], ei_affiliated[1]),
        ('P', 'P-P', 'P'): (ei_cites[0], ei_cites[1]),
    })
    # ('P', 'P-P', 'P'): (torch.cat([ei_cites[0], ei_cites[1]], dim=0), torch.cat([ei_cites[1], ei_cites[0]], dim=0))
    ...
    if len(args.extra_embedding):
        ...
        author_emb = torch.Tensor(g.num_nodes('A'), args.embed_size).uniform_(-0.5, 0.5)
        # topic_emb = torch.Tensor(g.num_nodes('field_of_study'), args.embed_size).uniform_(-0.5, 0.5)
        institution_emb = torch.Tensor(g.num_nodes('I'), args.embed_size).uniform_(-0.5, 0.5)

    with torch.no_grad():
        if features.shape[1] != args.embed_size:
            rand_weight = torch.Tensor(features.shape[1], args.embed_size).uniform_(-0.5, 0.5)
            features = features @ rand_weight
        if author_emb.shape[1] != args.embed_size:
            rand_weight = torch.Tensor(author_emb.shape[1], args.embed_size).uniform_(-0.5, 0.5)
            author_emb = author_emb @ rand_weight
        # if topic_emb.shape(1) != args.embed_size:
        #     rand_weight = torch.Tensor(topic_emb.size(1), args.embed_size).uniform_(-0.5, 0.5)
        #     topic_emb = topic_emb @ rand_weight
        if institution_emb.shape[1] != args.embed_size:
            rand_weight = torch.Tensor(institution_emb.shape[1], args.embed_size).uniform_(-0.5, 0.5)
            institution_emb = institution_emb @ rand_weight

    g.nodes['P'].data['feat'] = features
    g.nodes['A'].data['feat'] = author_emb
    g.nodes['I'].data['feat'] = institution_emb

    init_labels = torch.from_numpy(MAG240MDataset.paper_label)
    n_classes = MAG240MDataset.num_classes
    ...
    if symmetric:
        diag_name = os.path.join(args.emb_path, f'{args.dataset}_PPP_diag.pt')
        if not os.path.exists(diag_name):
            # PP = PP.to_symmetric()
            # assert torch.all(PP.get_diag() == 0)
            PPP_diag = sparse_tools.spspmm_diag_sym_AAA(PP)
    ...
    diag_name = os.path.join(args.emb_path, f'{args.dataset}_PAP_diag.pt')
    if not os.path.exists(diag_name):
        PAP_diag = sparse_tools.spspmm_diag_sym_ABA(PA)
        torch.save(PAP_diag, diag_name)
