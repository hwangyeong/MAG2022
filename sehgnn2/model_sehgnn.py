...
class SeHGNN_mag(nn.Module):
    ...
    def forward(self, feats_list, layer_feats, label_emb): #默认feats_dict的0号是P类型的属性
        tgt_feat = self.input_drop(feats_list[0])
        B = num_node = tgt_feat.size(0)
        x = self.input_drop(torch.stack(feats_list, dim=1))
        x = self.feat_project_layers(x)

        if self.label_feat_project_layers is not None:
            label_feats = self.input_drop(torch.stack(layer_feats, dim=1))
