...
from model_sehgnn import SeHGNN_mag
...
def load_data(datapath, feat_info, label_info, device=None):
    ...
    y_feat_all = []
    for i, (fn, _, _) in enumerate(label_info):
        logger.info("Loading label features for %d: %s ..." % (i, fn.upper()))
        label_feat = []
        fname = os.path.join(datapath, '%s.npy' % fn)
        logger.info("Loading %s" % fname)
        label_feat = torch.from_numpy(np.load(fname)).to(device, torch.float32)
        y_feat_all.append(label_feat)


    return x_all, y_all, y_feat_all

def feature_project(features, embed_size):
    for ii in range(len(features)):
        if features[ii].size(1) != embed_size:
            rand_weight = torch.Tensor(features[ii].size(1), embed_size).uniform_(-0.5, 0.5).to(features[ii].device)
            features[ii] = features[ii] @ rand_weight

def label_embedding(label_feats):
    ee = torch.stack(label_feats, dim=0)
    label_emb = ee.sum(0)
    label_emb = label_emb / len(label_feats)
    return label_emb

def preproces(stage, labels, probs_path, device, threshold, train_idx, valid_idx):
    if stage > 0:
        train_idx = torch.from_numpy(train_idx)
        valid_idx = torch.from_numpy(valid_idx)
        tr_va_te_nid = torch.cat([train_idx, valid_idx], dim=0)
        teacher_probs = torch.load(probs_path)
        confident_nid_inner = torch.arange(len(teacher_probs))[teacher_probs.max(1)[0] > threshold]
        extra_confident_nid_inner = confident_nid_inner[confident_nid_inner >= len(train_idx)]
        ...
        train_nid_with_pseudos = np.union1d(train_idx, confident_nid)
        print(f"enhanced train set number: {len(train_nid_with_pseudos)}")
        labels_with_pseudos[train_idx] = labels[train_idx]
        labels_with_pseudos[extra_confident_nid] = pseudo_labels[extra_confident_nid_inner]
    else:
        teacher_probs = None
        pseudo_labels = None
        labels_with_pseudos = labels.clone()
        confident_nid = train_idx
        train_nid_with_pseudos = train_idx
    return train_nid_with_pseudos, labels_with_pseudos
...
def parse_args(args=None):
    ...
    # parser.add_argument('probs_path', help='The directory of teacher_probs')
    ...
    parser.add_argument("--epoch_setting", nargs='+',type=int, default=[100, 100, 100])
...
if __name__ == '__main__':
    ...
    # args.output_path = "dataset_path/mplp_data/mplp/output_sehgnn"
    # args.gpu = True
    # args.finetune = True
    wandb_on = True

    if wandb_on:
        wandb.init(config={"current": "sehgnn"},
                project="MAG240M-OGB",
                entity="hwangyeong",
    ...
    label_feat_info = [
        ('x_pcbpcbp_ns_lratio', 153, 128),
        ('x_pcbpcp_ns_lratio', 153, 128),
        ('x_pcbp_ns_lratio', 153, 32),
        ('x_pcpcbp_ns_lratio', 153, 32),
        ('x_pcpcp_ns_lratio', 153, 32),
        ('x_pcp_ns_lratio', 153, 32),
        ('x_pwbawp_ns_lratio', 153, 32),
    ]

    logger.info("A Total of %d different type features" % len(feat_info))
    logger.info(feat_info)

    x_all, y_all, y_feat_all = load_data(args.input_path, feat_info, label_feat_info, device)
    feature_project(x_all, args.emb_size)
    label_emb = label_embedding(y_feat_all)
    ...
    mkdirs(output_path)
    probs_path = os.path.join(args.output_path, 'teacher_probs.pt')
    global_step = 0
    for thisrun in range(args.num_runs):
        print("-" * 100)
        print(f"Run {thisrun} start training")
        seed_everything(args.seed + thisrun)
        ...
        for stage in range(len(args.epoch_setting)):
            ...
            train_idx, labels = preproces(stage, y_all, probs_path, \
                device, args.threshold, train_idx0, valid_idx0)
            # train_idx = train_ikdx0
            valid_idx = valid_idx0
            logger.info("KFold: %d, Train: %d, Valid: %d" % (k, len(train_idx), len(valid_idx)))

            y_feats = [] if stage == 0 else y_feat_all
            ...
            model = SeHGNN_mag(args.emb_size, args.hidden, dataset.num_classes,
                len(x_all), len(y_feats),
            ...
            n_batch = len(train_idx) // args.batch_size
            for epoch in range(1, 1 + args.epoch_setting[stage]):

                model.train()
                total_loss = 0
                for ii, idx in enumerate(DataLoader(train_idx, args.batch_size, shuffle=True)): # train
                    optimizer.zero_grad()
                    # y_pred = model([x[idx].to(torch.float32) for x in x_all])
                    y_pred = model([x[idx].to(torch.float32) for x in x_all], [x[idx].to(torch.float32) for x in y_feats], label_emb[idx])
                    y_true = labels[idx]
                    ...
                    # if wandb_on:
                    #     global_step = (epoch-1) * n_batch + ii
                    #     wandb.log({"training_loss_step":loss.item()},step=global_step)
                ...
                with torch.no_grad(): # compute train accuracy
                    ...
                    for idx in DataLoader(train_idx, args.batch_size):
                        y = model([x[idx].to(torch.float32) for x in x_all], [x[idx].to(torch.float32) for x in y_feats], label_emb[idx])
                        y_pred.append(y.argmax(dim=-1).cpu())
                    y_pred = torch.cat(y_pred, dim=-1)
                    train_acc = evaluator.eval({'y_true': labels[train_idx].cpu(), 'y_pred': y_pred})['acc']
                ...
                with torch.no_grad(): # compute valid accuracy
                    ...
                    for idx in DataLoader(valid_idx, args.batch_size):
                        ...
                        y_pred.append(y.argmax(dim=-1).cpu())
                    y_pred = torch.cat(y_pred, dim=-1)
                valid_acc = evaluator.eval({'y_true': labels[valid_idx].cpu(), 'y_pred': y_pred})['acc']
                

                if wandb_on:
                    global_step = sum(args.epoch_setting) * thisrun + ((sum(args.epoch_setting[:stage]) + epoch ) if stage > 0 else epoch)
                    wandb.log({"training_train_acc":train_acc,
                            "training_valid_acc":valid_acc,
                            "training_loss": loss
                            },step=global_step)
            ...
            test_idx = np.concatenate([valid_idx0], axis=0)
            with torch.no_grad(): # compute test accuracy
                model.eval()
                y_pred = []
                probs = []
                for idx in DataLoader(test_idx, args.batch_size):
                    ...
                    probs.append(y)
                y_pred = torch.cat(y_pred, dim=-1)
            test_acc = evaluator.eval({'y_true': y_all[test_idx].cpu(), 'y_pred': y_pred})['acc']
            logger.info("Stage: %02d, Test_acc: %.4f" % (stage, test_acc))
            probs = torch.cat(probs, dim=0)
            torch.save(probs, probs_path)

    logger.info('Evaluating on valid set before fintune')
    ...
    with torch.no_grad():
        model.eval()
        y_pred = []
        for idx in DataLoader(valid_idx0, args.batch_size):
            y = model([x[idx].to(torch.float32) for x in x_all], [x[idx].to(torch.float32) for x in y_feat_all], label_emb[idx])
            y_pred.append(y.argmax(dim=-1).cpu())
        y_pred = torch.cat(y_pred, dim=-1)
    ...
    if args.finetune:
        ...
        best_valid_acc = 0
        global_step = 0
        n_batch = len(train_idx_ft) // args.batch_size
        for epoch in range(1, 30):
            ...
            for idx in DataLoader(train_idx_ft, args.batch_size, shuffle=True):
                optimizer.zero_grad()
                y_pred = model([x[idx].to(torch.float32) for x in x_all], [x[idx].to(torch.float32) for x in y_feat_all], label_emb[idx])
                y_true = y_all[idx]
                loss = F.cross_entropy(y_pred, y_true, weight=weight)
                loss.backward()
                optimizer.step()
            ...
            # if wandb_on:
            #     wandb.log({"finetuning_train_acc":train_acc,
            #                "finetuning_valid_acc":valid_acc,
            #                "finetuning_loss": loss},step=epoch)
