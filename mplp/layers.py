...
class MultiHeadLinear(nn.Module):
    ...
    def forward(self, x): # (N, Heads, d_in)
        # input size: [N, d_in] or [H, N, d_in] ??
        # output size: [H, N, d_out]
        if len(x.shape) == 3:
            x = x.transpose(0, 1) # (Head, N, d_in)

        x = torch.matmul(x, self.weight) # weight (Heads, d_in, hidden)
        if self.bias is not None:
            x += self.bias
        return x.transpose(0, 1) # return (N, Heads, d_in)
...
class GroupMLP(nn.Module):
    ...
    def forward(self, x):
        x = self.input_drop(x) # (N, in_dim)
        if len(x.shape) == 2:
            x = x.view(-1, 1, x.shape[1]) # (N, 1, in_dim)
        if self._residual:
            prev_x = x
        for layer_id, layer in enumerate(self.layers):
            x = layer(x) #多层MLP
