...
def parse_args(args=None):
    ...
    parser.add_argument("--epoch_setting", nargs='+',type=int, default=[200, 200, 200])
    ...
    for i in range(args.num_runs):
        ...
        for stage in range(len(args.epoch_setting)):
            ...
            for epoch in range(1, 1 + args.epoch_setting[stage]):
                ...
                if wandb_on:
                    global_step = sum(args.epoch_setting) * i + args.epoch_setting[stage-1] + epoch if stage > 0 else epoch
            ...
            test_idx = np.concatenate([train_idx0, valid_idx0], axis=0)
            with torch.no_grad(): # compute test accuracy
                ...
                for idx in DataLoader(test_idx, args.batch_size):
                    ...
                    probs.append(y)
                y_pred = torch.cat(y_pred, dim=-1)
            test_acc = evaluator.eval({'y_true': labels[test_idx].cpu(), 'y_pred': y_pred})['acc']
