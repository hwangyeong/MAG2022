...
def save_col_slice(x_src, x_dst, start_row_idx, end_row_idx, start_col_idx,
    ...
    chunk, offset = 50000, start_row_idx
...
class MAG240M(object):
    ...
    def prepare_data(self):
        ...
        if not osp.exists(path):
            ...
            # author
            edge_index = dataset.edge_index('author', 'writes', 'paper') #(2, 386022720)
            edge_index = edge_index.T #(386022720, 2)
            row, col = edge_index[:, 0], edge_index[:, 1]
            edge_index = np.stack([col, row], axis=1) #(386022720, 2)
            log.info(edge_index.shape)
            author_graph = BiGraph(edge_index, dst_num_nodes=dataset.num_authors) #有向图？paper指向author
